...
void onInit(CRules@ this)
{
	...
	this.set_CBitStream("aw_serialised_team_hud", stream);
}
...
void onRender(CRules@ this)
{
	...
	this.get_CBitStream("aw_serialised_team_hud", serialised_team_hud);
	...
	if (serialised_team_hud.getBytesUsed() > 10)
	{
		...
		if (serialised_team_hud.saferead_u16(check) && check == 0x5afe)
		{
			...
			{
				AW_HUD hud(serialised_team_hud);
				...
				//if (hud.spawn_time != 255)
				{
					...
					time += "    Base Health:" + hud.base_health;
	...
	string propname = "aw spawn time " + p.getUsername();
